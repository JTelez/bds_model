...
chole.raw <- read_excel("Datasheet_SS_ESGE_added_updated_2Tests.xlsx",
                        sheet = 1, col_types = c(rep("numeric", 2), rep("logical", 2),
                                                 rep("numeric", 8), rep("logical", 7)))
...
mind = as.logical(apply(as.matrix(is.na(chole.raw[4:11])), 1, prod))

table(apply(as.matrix(is.na(chole.raw[5:12])), 1, sum))
sapply(chole.raw, function(s){sum(is.na(s))})

chole.sub = chole.raw[!is.na(chole.raw$Age), 1:11]
...
chole.mice[!is.na(chole.mice$Age), 1:11] = imp2
...
train.glm = function(data){
  ...
  tdata = na.omit(tdata)[c(1:11,13,16)]
...
train.rf = function(data){
  ...
  # tdata = data %>% dplyr::filter(!Ascending_cholangitis)
  tdata = na.omit(tdata)[c(1:11,13,16)]
  randomForest(factor(BDS) ~ ., tdata, ntree = 1000, mtry=2)
}
...
ggsave("ROCs_2Tests.tiff", p1, width = 8, height = 6, compression = "lzw")
```
