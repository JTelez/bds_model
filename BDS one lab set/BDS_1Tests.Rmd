...
chole.raw <- read_excel("Datasheet_SS_ESGE_added_updated_1Tests.xlsx",
...
asge.sensitivity.se = 1.96 * sqrt((asge.sensitivity - (1-asge.sensitivity))/sum(chole.raw$BDS))
print(asge.sensitivity)
print(asge.sensitivity.se)

asge.specificity = sum(1-chole.raw$BDS[prediction.asge == 0]) / sum(1-chole.raw$BDS)
asge.specificity.se = 1.96 * sqrt((asge.specificity - (1-asge.specificity))/sum(1-chole.raw$BDS))
print(asge.specificity)
print(asge.specificity.se)

roc.asge = roc(as.numeric(chole.raw$BDS), prediction.asge, ci=TRUE)
print(roc.asge$auc)
print(roc.asge$ci)
print(ci(roc.asge))
...
roc.uasge = roc(as.numeric(chole.raw$BDS), prediction.uasge, ci=TRUE)
print(roc.uasge$auc)
print(roc.uasge$ci)
...
roc.esge = roc(as.numeric(chole.raw$BDS), prediction.esge, ci=TRUE)
print(roc.esge$auc)
print(roc.esge$ci)

print(table(ERCP=chole.raw$ESGE.high.risk, Stone=chole.raw$BDS))
print(roc.test(roc.asge, roc.uasge))
print(roc.test(roc.uasge, roc.esge))
print(roc.test(roc.asge, roc.esge))
...
cv = function(ftrain, fpredict, data, fold=5, verbose=FALSE) {
  ...
  if(verbose){
    ...
    cat(sprintf("Mean AUC CI: %f for %d fold cross validation \n", mean(AUCs_ci), fold)) 
  }
...
ntimes = function(iter, train, pred, data){
  ...
  list(prediction = pred_ind, roc = roc(data$BDS, pred_ind), auc = median(res), auc_ci = roc(data$BDS, pred_ind, ci=TRUE)$ci)
}
```
...
ggsave("ROCs_1Tests.tiff", p1, width = 8, height = 6, compression = "lzw")
```
